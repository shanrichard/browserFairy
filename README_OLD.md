...
### 🎛 可配置输出与目录（新增）
- 用途：减少无关数据、便于 AI/人工快速聚焦问题，并将数据保存到项目可访问的位置。
- 参数（前台综合监控 `--monitor-comprehensive` 支持）：
  - `--data-dir`: 指定数据输出目录（默认 `~/BrowserFairyData`，同环境变量 `BROWSERFAIRY_DATA_DIR`）。
  - `--output`: 选择要收集的数据类型（预设或逗号分隔自定义列表）。
- 预设模式：
  - `all`: 收集全部数据（默认，保持向后兼容）
  - `errors-only`: 仅错误相关（`console:error`, `exception`, `network:failed`）
  - `performance`: 性能相关（`memory`, `gc`, `network:complete`）
  - `minimal`: 最小化输出（`console:error`, `exception`）
  - `ai-debug`: AI 调试常用（`console:error`, `exception`, `network:failed`, `memory`）
- 自定义列表（示例）：
  - `--output console:error,exception,network:failed`
  - 支持细粒度：`network:start`, `network:complete`, `network:failed`；`console` 的级别包含 `error|warn|log`。
- 兼容性与行为：
  - 不传 `--output` 等价于 `all`；默认行为不变。
  - 过滤在数据回调层执行，不改变原始字段与写入格式，只决定“是否写入”。
  - 当前后台模式（`--daemon`）暂不支持 `--data-dir/--output` 参数，需前台运行或使用 `BROWSERFAIRY_DATA_DIR` 设置目录。
...
# 性能分析：只收集内存、GC、已完成的网络请求
browserfairy --monitor-comprehensive \
  --data-dir ./perf \
  --output performance

# 自定义细粒度：控制台错误+异常+失败的网络请求
browserfairy --monitor-comprehensive \
  --data-dir ./debug \
  --output console:error,exception,network:failed

# 显式收集全部（与默认一致）
browserfairy --monitor-comprehensive --output all
```
...
| 监控维度 | 具体指标 | 检测能力 |
|---------|---------|---------|
| 🧠 **内存监控** | JS Heap, DOM节点, 事件监听器详情分析 | 内存泄漏、DOM累积、监听器泄漏定位 |
