...
# 采样配置常量
HEAP_SAMPLING_INTERVAL = 32768  # bytes, Chrome默认
HEAP_PROFILE_COLLECTION_INTERVAL = 30.0  # seconds, 比内存收集慢6倍  
HEAP_SAMPLING_DURATION_LIMIT = 300.0  # seconds, 5分钟重启
...
async def _enable_comprehensive_monitoring(self):
    ...
    # 新增：HeapSampling监控器初始化（遵循完全相同的模式）
    self.heap_sampling_monitor = HeapSamplingMonitor(self.connector, self.session_id, self.event_queue, self.status_callback)
...
**基于现有DataManager模式，仅需一行代码修改**：

现有的DataManager._handle_data_callback()方法已能自动处理任何类型事件，只需添加heap_sampling类型分发：
